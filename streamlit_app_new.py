...
BU_text = "The Business Understanding phase focuses on understanding the objectives and requirements of the project. Our Goal was to adopt deep learning for medical image applicatons." \
          "So the main focus was to develop an accurate model to detect using a chest x-ray if the person has pneumonia or not"
DU_text = """In the Data Understanding phase the focus is to identify, collect, and analyze the data sets that can help you accomplish the project goals." \
            So first we tried to understand how the Data we have is structured.
            The Kaggle dataset which contains 5683 X-Ray images (JPEG).
            The dataset is categorized into 3 which are training, testing, and validation, each image category consists of subfolders
            like Normal and Pneumonia. Clearly, chest X-ray images (anterior-posterior) have been examined by the review accomplices of
            pediatric patients within the age group (1 to 5 years) collected from Guangzhou Women and Children Medical Center,
            Guangzhou, Southern China. We took all chest X-ray imaging and applied them as a major aspect of patients' normal clinical
            consideration.
          """
DP_text = """ 
            ...
            we need to expand artificially our dataset. We can make your existing dataset even larger. The idea is to alter the training data
            with small transformations to reproduce the variations. Approaches that alter the training data in ways that change the
            array representation while keeping the label the same are known as data augmentation techniques.
...
Mod_text = """ For the Modeling Phase we decided on building a Convolutional Neural Network.
            ...
            computer vision tasks like localization and segmentation, video analysis, to recognize obstacles in self-driving cars,
            as well as speech recognition in natural language processing. As CNNs are playing a significant role in these fast-growing
            ...
            the implementation of neural networks easy. It also supports multiple backend neural network computation like tensorflow which
            also used for our project.TensorFlow is an open-source end-to-end platform for creating Machine Learning applications.
            It is a symbolic math library that uses dataflow and differentiable programming to perform different tasks focused
            on training and inference of deep neural networks. It allows developers to create machine learning applications using various
            tools, libraries, and community resources. Essentialy we used Tensorflow, because it speeds up the process of training a model!   
        """
...
with st.expander("Data Understanding"):
    st.text(DU_text)
    st.text("""
        as you can see from the diagrams we had a small problem, the data is imbalanced in the training Data the size of pictures
